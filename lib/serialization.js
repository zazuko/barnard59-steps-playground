...
async function turtleToQuads (turtle) {
  const input = new Readable({
    read: () => {
      input.push(turtle)
      input.push(null)
...
function quadToString(quad){
  return `quad(${quad.subject.value},${quad.predicate.value},${quad.object.value},${quad.graph?.value})`
}
...
async function getQuadsAndInfo (stream) {
  ...
  for await (const chunk of stream) {
    if (isIterable(chunk)) {
      ...
      quads.push(chunk)
      info.push(quadToString(chunk))
    }
  }
  return { info, quads }
}

export { jsonToQuads, quadsToJson, getQuadsAndInfo, readableFromArray, turtleToQuads }
