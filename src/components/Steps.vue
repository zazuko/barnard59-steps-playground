...
async function loadResources () {
  const quads = await rdfEditor.value.quads
...
function onPrefixesParsed (e) {
  prefixes.value = e.detail.prefixes
}
    ...
    <div class="bg-white shadow-sm flex justify-between items-stretch">
      <div class="px-4 py-2 shadow-md flex items-center gap-4">
        <div class="flex items-center gap-2">
          <a href="https://zazuko.com" rel="noopener noreferer" target="_blank">
        ...
        <Splitpanes horizontal>
          <Pane>
            <div class="flex-grow flex flex-col">
              <header class="px-4 py-2 bg-gray-200 flex items-center justify-between">
                <h2 class="font-bold text-lg">Graph</h2>
                <!-- Hack to fix header height :shrug: -->
                <input class="input invisible" readonly/>
              </header>
              <GraphView :dataset="dataset" :env="env" class="flex-grow"/>
            </div>
          </Pane>
        </Splitpanes>
      </Pane>
